...
## ADD THE MEAN DIFF, ORS, COHEN'S D, WITH ASSOCIATED CIS
## =============================================================================
...
.fu_or_multinom <-
    ...
    ~ .x %>%
      map(
        ~ .x %$%
          ...
          select(y.level:estimate, conf.low:conf.high, p.value) %>%
          mutate(
  ...
  unite(cont, c(cont, c)) %>%
  filter(!cont %in% "x3_1") %>%
  mutate(cont = str_replace(cont, "(x)(\\d)(_)(\\d)", "\\1\\4\\3\\2")) %>%
  rename(
    md_or = or,
    md_or_l = conf.low,
    md_or_u = conf.high,
  ...
  pivot_wider(names_from = cont, values_from = md_or:last_col()) %>%
  rename_with(
    ~ str_replace_all(.x, c("(md_or)(_u|_l)(_x\\d_\\d)" = "\\1\\3\\2")),
    starts_with(c("md_or_u_", "md_or_l_"))
  ) %>%
  select(var, term, contains(c("x2_1", "x2_3", "x3_1"))) %>%
  relocate(starts_with("p_"), .after = last_col())
...
.fu_d_df <-
  ...
  select(
    var3, value3, test,
    starts_with(c("md_or_x2_1", "md_or_x3_1", "md_or_x2_3")),
    starts_with(c("d_x2_1", "d_x3_1", "d_x2_3")),
    starts_with("p_or"),
    everything()
  )

## GET PRINTED VERSION OF MD_OR AND D BY PIVOTING LONGER
## -----------------------------------------------------------------------------
...
fu_test <-
  ...
  arrange(order) %>%
  mutate(
    p_md_or_x2_1 = case_when(
      p < 0.05 & abs(x2_1) < bonf ~ paste0("<b>", md_or_x2_1, "</b> [", md_or_x2_1_l, ", ", md_or_x2_1_u, "]"), # nolint
      p < 0.05 & abs(x2_1) > bonf ~ paste0(md_or_x2_1, " [", md_or_x2_1_l, ", ", md_or_x2_1_u, "]"), # nolint
      TRUE ~ NA_character_
    ),
    p_md_or_x3_1 = case_when(
      p < 0.05 & abs(x3_1) < bonf ~ paste0("<b>", md_or_x3_1, "</b> [", md_or_x3_1_l, ", ", md_or_x3_1_u, "]"), # nolint
      p < 0.05 & abs(x3_1) > bonf ~ paste0(md_or_x3_1, " [", md_or_x3_1_l, ", ", md_or_x3_1_u, "]"), # nolint
      TRUE ~ NA_character_
    ),
    p_md_or_x2_3 = case_when(
      p < 0.05 & abs(x3_2) < bonf ~ paste0("<b>", md_or_x2_3, "</b> [", md_or_x2_3_l, ", ", md_or_x2_3_u, "]"), # nolint
      p < 0.05 & abs(x3_2) > bonf ~ paste0(md_or_x2_3, " [", md_or_x2_3_l, ", ", md_or_x2_3_u, "]"), # nolint
      TRUE ~ NA_character_
    ),
    p_d_x2_1 = case_when(
      p < 0.05 & abs(x2_1) < bonf ~ paste0("<b>", d_x2_1, "</b> [", d_x2_1_l, ", ", d_x2_1_u, "]"), # nolint
      p < 0.05 & abs(x2_1) > bonf ~ paste0(d_x2_1, " [", d_x2_1_l, ", ", d_x2_1_u, "]"), # nolint
      TRUE ~ NA_character_
    ),
    p_d_x3_1 = case_when(
      p < 0.05 & abs(x3_1) < bonf ~ paste0("<b>", d_x3_1, "</b> [", d_x3_1_l, ", ", d_x3_1_u, "]"), # nolint
      p < 0.05 & abs(x3_1) > bonf ~ paste0(d_x3_1, " [", d_x3_1_l, ", ", d_x3_1_u, "]"), # nolint
      TRUE ~ NA_character_
    ),
    p_d_x2_3 = case_when(
      p < 0.05 & abs(x3_2) < bonf ~ paste0("<b>", d_x2_3, "</b> [", d_x2_3_l, ", ", d_x2_3_u, "]"), # nolint
      p < 0.05 & abs(x3_2) > bonf ~ paste0(d_x2_3, " [", d_x2_3_l, ", ", d_x2_3_u, "]"), # nolint
      TRUE ~ NA_character_
    ),
    ) %>%
  group_by(var3, value3) %>%
  mutate(
    p_d_x2_1_fx = case_when(
      grepl(d_x2_1_l_fx, d_x2_1_u_fx) ~ d_x2_1_l_fx,
      !grepl(d_x2_1_l_fx, d_x2_1_u_fx) && var3 %in% "isas_3a" ~ paste(d_x2_1_u_fx, "to", d_x2_1_l_fx), # nolint
      TRUE ~ paste(d_x2_1_l_fx, "to", d_x2_1_u_fx),
    ),
    p_d_x3_1_fx = case_when(
      grepl(d_x3_1_l_fx, d_x3_1_u_fx) ~ d_x3_1_l_fx,
      !grepl(d_x3_1_l_fx, d_x3_1_u_fx) && var3 %in% "isas_3a" ~ paste(d_x3_1_u_fx, "to", d_x3_1_l_fx), # nolint
      TRUE ~ paste(d_x3_1_l_fx, "to", d_x3_1_u_fx),
    ),
    p_d_x2_3_fx = case_when(
      grepl(d_x2_3_l_fx, d_x2_3_u_fx) ~ d_x2_3_l_fx,
      !grepl(d_x2_3_l_fx, d_x2_3_u_fx) && var3 %in% "isas_3a" ~ paste(d_x2_3_u_fx, "to", d_x2_3_l_fx), # nolint
      TRUE ~ paste(d_x2_3_l_fx, "to", d_x2_3_u_fx),
    )
  ) %>%
  ungroup()
  ...
  list(
    full =
      lpa_t0t1_outc_w %>%
        ...
        split(.$var) %>%
        map(~ .x %$% DescTools::MeanDiffCI(t1, t0, na.rm = TRUE)) %>%
        bind_rows(.id = "var") %>%
        mutate(c = "0"),
    c123 =
      lpa_t0t1_outc_w %>%
...
.lpa_meandiff_desc <-
  .lpa_meandiff_df %>%
  select(-.drop) %>%
  rename(md = meandiff, md_l = lwr.ci, md_u = upr.ci) %>%
...
.lpa_prepost_desc <-
  ...
  pivot_longer(
    -c(c, .drop),
    names_to = c("var", "time", "type"),
    names_sep = "_"
  ) %>%
  mutate(across(c(value, time), ~ as.numeric(.x))) %>%
  pivot_wider(
    names_from = c(type),
    values_from = value
  ) %>%
...
## START THE PUBLICATION TABLE OF T0 VS T1 DATA
.lpa_prepost_kbl <-
  .lpa_prepost_desc %>%
  full_join(.lpa_prepost_ttest) %>%
  full_join(.lpa_meandiff_desc) %>%
  full_join(.lpa_prepost_d) %>%
  select(-.drop)

lpa_prepost_kbl_print <-
  .lpa_prepost_kbl %>%
  mutate(
    across(
      c(m, sd, t_value, md, md_l, md_u, d, d_l, d_u),
      ~ sprintf("%.2f", .x)
    ),
    m_sd = paste0(m, "(", sd, ")"),
    t_df = case_when(
    ...
    t_df2 = paste0("*t*(", dfs, ")", " = ", t_value, ", ", p_print),
    var_t = case_when(
...
lpa_prepost_kbl <-
  lpa_prepost_kbl_print %>%
  select(order, c, time, var, var_t, n, m_sd, t_df, md, d, d_fx, d_ci_fx) %>% # nolint
  pivot_wider(
    names_from = c,
...
lpa_pp_long <-
  lpa_prepost_kbl_print %>%
  select(c, time, m, sd, var, t_df2, d_ci_fx, d_fx)
...
## plot of variables with sig diffs at baseline
pd <- position_dodge(0.01) # move them .05 to the left and right

.fu_plot_names <-
  c(
    "TB", "PB", "BHS", "AC", "DEP", "ANX", "STR",
    "DSH[hx]^a", "DSH[n]", "DSH[age]", "ISAS[intra]",
    "SI[hx]^a", "MSSI"
  )
...
.fu_cont_plot <-
  ...
  select(c, contains("_n")) %>%
  mutate(
    across(everything(), ~ str_remove(.x, "_n")),
  ...
  summarise(
    across(
      ...
      list(
        ...
        ci = ~ (sd(.x, na.rm = TRUE) / sqrt(length(.x))) * qt(0.95/2 + .5, length(.x) - 1) # nolint
      ),
      .names = "{.col}.{.fn}")
  ) %>%
  ungroup() %>%
...
.fu_cat_plot <-
  lpa_t0 %>%
  select(c, hxdsh, hxsuicide) %>%
  mutate(across(starts_with("hx"), ~ as.numeric(.x) - 1)) %>%
  group_by(c) %>%
  summarise(N = n(), across(starts_with("hx"), ~ sum(.x)/length(.x))) %>%
  pivot_longer(-c(c, N)) %>%
  rename(value.mean = value, value.N = N)

.fu_plot_df <-
  full_join(.fu_cont_plot, .fu_cat_plot) %>%
  group_by(name) %>%
  mutate(
    ID = case_when(
      name %in% "tb"          ~ 1,
      ...
      name %in% "dsh_n"       ~ 9,
      name %in% "isas_3a"     ~ 10,
      name %in% "isas_intra"  ~ 11,
      name %in% "hxsuicide"   ~ 12,
      name %in% "si"          ~ 13
    ),
    .before = name,
    name = factor(
      ...
      levels = c(
        "tb", "pb", "bhs", "ac", "dep", "anx", "str",
        "hxdsh", "dsh_n", "isas_3a", "isas_intra",
        "hxsuicide", "si"
      )
    )
  ) %>%
...
lpa_fig_plot_all <-
  ...
  geom_line(aes(x = ID, y = value.mean), position = pd) +
  ...
  labs(
    x = "Variables at Baseline with Significant Differences Between Classes",
    y = "Standardised Means",
    colour = "Class",
    shape = "Class",
    linetype = "Class",
    fill = "Class"
  ) +
  scale_x_discrete(label = parse(text = .fu_plot_names))

ggsave(
  "./figures/lpa_fig_plot_all.png",
  lpa_fig_plot_all
)
